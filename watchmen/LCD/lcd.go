package LCD

import (
	"os"
	"image"
	_ "image/jpeg"
	_ "image/png"
    "math"
    "github.com/kakaryan/i2cssd1306"
)

type LcdWrapper struct {
    Dev     *i2cssd1306.SSD1306
    Height  int
    Width   int
    
}

type Buf struct {
	Pix []byte
	Width, Height int
}

var (
    font []byte = []byte{,}
)


func newLcdWrapper(iicBussAddr, devAddr *byte, height, width *int) *LcdWrapper {
    lcdDev := i2cssd1306.NewDevice()
    lcdDev.Init(*iicBussAddr, *devAddr, *height, *width)
    lcdDev.InitDevice()
    return &LcdWrapper{
        Dev: lcdDev,
        Height: *height,
        Width: *width,
    }
}

func imgToBitmap(fname *string) (buf *Buf, err error) {
	var f *os.File
	if f, err = os.Open(*fname); err != nil {
		return
	}

	var img image.Image
	img, _, err = image.Decode(f)
	if err != nil {
		return
	}
	rect := img.Bounds().Max

	h := (rect.Y + 7) / 8

	buf = &Buf{
		Width: rect.X,
		Height: rect.Y,
		Pix: make([]byte, rect.X*h),
	}

	for i := 0; i < h; i++ {
		for j := 0; j < rect.X; j++ {
			by := uint32(0)
			for k := 0; k < 8; k++ {
				c := img.At(j, i*8+k)
				r,g,b,_ := c.RGBA()
				y := (r+g+b)/3
				if y < 65536/2 {
					by |= 1<<uint32(k)
				}
			}
			buf.Pix[i*buf.Width+j] = byte(by)
		}
	}

	return
}

func (w *LcdWrapper) writeChar(char *byte, pos *int) {
	fontOffset := int(*char) * 5
	for i := 0; i <= 4; i++ {
		charSlice := font[fontOffset + i]
		w.Dev.WriteData(charSlice, *pos + i)
	}
}

func (w *LcdWrapper) WriteText(text string, pos int) {
	for _, char := range []byte(text) {
        w.writeChar(&char, &pos)
		pos += 6
    }
}

func (w *LcdWrapper) WriteImage(fileName string, pos int) {
	img, err := imgToBitmap(&fileName)
    if err != nil {
        panic(err)
    }
	var currPos, currRow int
    for _, val := range img.Pix {
		if currPos % img.Width == 0 {
			currPos = 0
			currRow++
		}
		w.Dev.WriteData(val, pos + currPos + (w.Width * (currRow - 1)))
		currPos++
    }
}

func (w *LcdWrapper) WriteHLine(pos, len int) {
    for i := 0; i < len; i += 1 {
        w.Dev.WriteData(0x10, i + pos)
    }
}

func (w *LcdWrapper) WriteVLine(pos, rows int) {
    for i := 0; i < w.Width * rows; i += w.Width {
        w.Dev.WriteData(0xFF, pos + i)
    }
}

func (w *LcdWrapper) WriteHDots(pos, len int) {
    for i := 0; i < len; i += 2 {
        w.Dev.WriteData(0x10, i + pos)
    }
}

func (w *LcdWrapper) WriteVDots(pos, rows int) {
    for i := 0; i < w.Width * rows; i += w.Width {
        w.Dev.WriteData(0xAA, pos + i)
    }
}

func (w *LcdWrapper) ClearArea(pos, len, rows int) {
    for row := 0; row < rows; row++ {
        for i := 0; i < len; i++ {
            w.Dev.WriteData(0x00, (row * w.Width) + pos + i)
        }
    }
}

func (w *LcdWrapper) ClearImageArea(fileName string, pos int) {
    img, err := imgToBitmap(&fileName)
    if err != nil {
        panic(err)
    }
    rows := math.Ceil(float64(img.Height) / 8.0)
    w.ClearArea(pos, img.Height, int(rows))
}